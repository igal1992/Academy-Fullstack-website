...
class register extends React.Component {
    ...
    async componentDidMount(){
        ...
        try{
            ...
            const response_accountDescription = await fetch(`https://nodejs-blacknightacademy-site.herokuapp.com/api/accountDescription/getData/`); 
    ...
    registerStudent = (e) => {
        e.preventDefault();
            ...
            fetch("https://nodejs-blacknightacademy-site.herokuapp.com/api/register/student",requestOptions)
            .then(response => {
                if (response.ok !== true) {
                    this.setState({
                        username:""
    ...
    registerInstructor = (e) => {
        e.preventDefault();
            ...
            fetch("https://nodejs-blacknightacademy-site.herokuapp.com/api/register/instructor",requestOptions)
            .then(response => {
                if (response.ok !== true) {
                    this.setState({
                        username:""
            ...
            <div className={"login-register-component"}>
                ...
                <div className={"register"}>
                    ...
                    <form className = {"student-method"} style = {{display: this.state.studentMeth}} onSubmit = {this.registerStudent}>
                        ...
                        <div className={"inputField"}><input name={account.name} placeholder={account.placeholder} onChange={this.update} value={this.state[account.value]} type={account.type} pattern={account.pattern} required title ={account.hint}/>
                        ...
                        <select name = "courses[]" className = "register_classes" onChange = {this.updateClass} required>
                            ...
                            <optgroup label = {faculty.department_name}>
                                    {faculty.courses.map((classes) =>{return(
                                    <option value = {classes.course_name}>{classes.course_name}</option>)})}
                                </optgroup>)})}
                        </select><br/><br/> 
                    ...
                    <form className = {"instructor-method"} style = {{display: this.state.instructorMeth}} onSubmit={this.registerInstructor}>
                        ...
                        <select name = "courses[]" className = "register_classes" onChange = {this.updateClass} required>
                            ...
                            {this.state.faculties.map((faculty) =>{return(<option value = {faculty.department_name}>{faculty.department_name}</option>)})}
                        </select><br/><br/> 
                        <input className={"login-register"} type = "submit" value = "Register"/>
                    </form>
                </div>
