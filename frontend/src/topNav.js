...
class TopNav extends React.Component{
    ...
    async componentDidMount(){
        ...
        try{
            const response_course = await fetch("https://nodejs-blacknightacademy-site.herokuapp.com/api/course/getData");
            const response_coursesByName = await fetch(`https://nodejs-blacknightacademy-site.herokuapp.com/api/course/getDataByDepartmentName/`+ engi_faculty); 
            const response_department = await fetch("https://nodejs-blacknightacademy-site.herokuapp.com/api/department/getData");
    ...
    confirmDelete=()=>{
        ...
        if(this.state.courseOfUser){
            fetch("https://nodejs-blacknightacademy-site.herokuapp.com/api/login/student",requestOptions)
            .then(response => {
                if(response.status === 200){
                    fetch(`https://nodejs-blacknightacademy-site.herokuapp.com/user/deleteUser/` + username, { method: 'POST' });
                    Cookies.remove("user");
                    var history = this.props.history;
                    window.location.reload(history.push(""));
                }else{
        ...
        fetch("https://nodejs-blacknightacademy-site.herokuapp.com/api/login/instructor",requestOptions)
        .then(response => {
            if(response.status === 200){
                fetch(`https://nodejs-blacknightacademy-site.herokuapp.com/user/deleteUser/` + username, { method: 'POST' });
                Cookies.remove("user");
                        ...
                        <div className = {"subNavBtn"}>
                            {this.state.faculties.map((faculty)=> {return (
                                <div className={"faculty"}>
                                    ...
                                    {faculty.courses.map((school) => {return(<div className={"styled-buttons"}><i className="fa fa-share" aria-hidden="true"/> <strong>B.{school.type}</strong> in {school.course_name}<p className={"professor"}>{school.instructor_name}</p></div>)})}
                                </div>)}
                            )}
                        </div>
                    </li>
                        ...
                        <div className = {"subNavBtn-master"}>
                            {this.state.faculties.map((faculty)=>{return(
                                <div className={"faculty"}>
                                    ...
                                    {faculty.courses.map((school) => {return(<div className={"styled-buttons"}><i className="fa fa-share" aria-hidden="true"/> <strong>M.{school.type}</strong> in {school.course_name}<p className={"professor"}>{school.instructor_name}</p></div>)})}
                                </div>
                            )})}
                        </div>
                    </li>
                    <li className = {"dropdowns"}>
                        ...
                        <div className = {"subNavBtn-engi"}>
                            <div className={"faculty"}>
                                ...
                                {this.state.Engi.map((school) => {return(<div className={"styled-buttons"}><i className="fa fa-share" aria-hidden="true"/>{school.course_name}<p className={"professor"}>{school.prof}</p></div>)})}
                            </div>
                        </div>
                    </li>
                </ul>
