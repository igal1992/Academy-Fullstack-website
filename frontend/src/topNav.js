...
class TopNav extends React.Component{
    ...
    async componentDidMount(){
        ...
        try{
            const response_course = await fetch("https://blacknightacademynew-backend.herokuapp.com/api/course/getData");
            const response_coursesByName = await fetch(`https://blacknightacademynew-backend.herokuapp.com/api/course/getDataByDepartmentName/`+ engi_faculty); 
            const response_department = await fetch("https://blacknightacademynew-backend.herokuapp.com/api/department/getData");
    ...
    confirmDelete=()=>{
        ...
        if(this.state.courseOfUser){
            fetch("https://blacknightacademynew-backend.herokuapp.com/api/login/student",requestOptions)
            .then(response => {
                if(response.status === 200){
                    fetch(`https://blacknightacademynew-backend.herokuapp.com/user/deleteUser/` + username, { method: 'POST' });
                    Cookies.remove("user");
                    var history = this.props.history;
                    window.location.reload(history.push(""));
                }else{
        ...
        fetch("https://blacknightacademynew-backend.herokuapp.com/api/login/instructor",requestOptions)
        .then(response => {
            if(response.status === 200){
                fetch(`https://blacknightacademynew-backend.herokuapp.com/user/deleteUser/` + username, { method: 'POST' });
                Cookies.remove("user");
